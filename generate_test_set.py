...
for sps in [0,1,2,3,4]:
    ...
    for f in fs:
        ...
        np.save(f"splits/{args.split}_control_test_better/control_test{f}.npy", np.array(tasks, dtype=object), allow_pickle=True)
        # np.save(f"splits/torch_control_test.npy", np.array(tasks, dtype=object), allow_pickle=True)
        del(test_gen)
        del(tasks)
        gc.collect()
