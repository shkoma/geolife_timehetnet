...
for id in user_list['user_id']:
    ...
    user_full_df = pd.concat([user_full_df, full_df], axis=0).reset_index(drop=True).copy()
        # full_df = pd.concat([full_df.iloc[:, :3], pd.get_dummies(full_df.iloc[:, 3:], columns=['year', 'month', 'week', 'hour'], drop_first=True).astype(np.int64)], axis=1)
...
if mode == 'mlp':
    user_full_df = pd.concat([user_full_df.iloc[:, :3], pd.get_dummies(user_full_df.iloc[:, 3:], columns=['year', 'month', 'week', 'hour'], drop_first=True).astype(np.int64)], axis=1)

total_samples = user_full_df.shape[0] // divided_len
train_samples = int(total_samples * train_ratio)
user_full_train_df = user_full_df.iloc[:train_samples * divided_len, :].copy().reset_index(drop=True)
user_full_test_df = user_full_df.iloc[train_samples * divided_len:, :].copy().reset_index(drop=True)

user_full_df.to_csv('user_full_file.csv', index=False)
user_full_train_df.to_csv('user_full_train.csv', index=False)
user_full_test_df.to_csv('user_full_test.csv', index=False)

print(user_full_df.head(2))
print(f"mode: {mode}")
print(f"users: {user_list.shape[0]}, user_list: {user_list['user_id'].to_list()}")
print(f"user_full: {user_full_df.shape[0]//divided_len}")
print(f"user_full_train: {user_full_train_df.shape[0]//divided_len}")
print(f"user_full_test: {user_full_test_df.shape[0]//divided_len}")
...
time_df.to_csv('full_user_list.csv')