...
#         print (idxs)
        ...
        while True:
            ...
            if shuffle and mode != 2: # 2 == test
                random.shuffle(idxs)
            
            for idx in idxs:
                # idx = data_set[idx], 즉, data_set file 지정, 거기서 sample 추출
                # tasks_x, tasks_y 는 각 data_set에서 sample 추출한 data에서 X, y 를 지정
                # y 는 각 mini_batch 의 맨 끝 row, 맨 끝 column 값(하나의 값)
                task_x, new_feat_size = sampleTaskControl(self.data[mode], idx,
                                                          cur_length, cur_min, cur_max,
                                                          n_samples, augment=False)
                ...
                if task_x is not None:
                    ...
                    # shape (batch, 1), ex) (10, 1) 각 mini-bath 의 y 값
            ...
            # while True:
            # (batch, mini-batch, y(row), x(column-feature))
            yield ((np.array(tasks_x)[:,:n_samples_q,:,:], # query X
                    np.array(tasks_x)[:,n_samples_q:,:,:], # supp  X
                    np.array(tasks_y)[:,n_samples_q:]),    # supp  Y
                    np.array(tasks_y)[:,:n_samples_q])     # query Y
            #   query set X:(5, 3, 3, 3)
