...
class ConvBlock(nn.Module):
    ...
    def forward(self, inp): # inp: (5, 10, 15, 120, 257)
        shape = torch._shape_as_tensor(inp)
        x = torch.reshape(inp, [-1, shape[-2], shape[-1]])
        x = x.transpose(-2, -1) # (750, 257, 120), (5, 256, 120)
    ...
    for idx, n in enumerate(dims):
        if final and idx == (len(dims) - 1):
            ...
            if begin and idx == 0: # begin 의 input_shape 변경이 필요할 듯
                ...
                final_list.append(My_Linear((dims[-2] + 1), dims[idx], name=f"{name}-{idx}"))
            elif final and idx == 0:
                final_list.append(My_Linear(dims[-2]*2, dims[idx], name=f"{name}-{idx}"))
            else:    
...
class TimeHetNet(nn.Module):
    def __init__(self,
        ...
        self.out_features = output_shape[0] # time
    ...
    def forward(self, inp):
        ...
        # in_xs: (5, 10, 12, 120, 257)
        in_xs = self.time_fv(in_xs)
        ...
        # sup_y_1:(5, 10, 6, 2, 1)
        in_ys = torch.concat([sup_y_1, in_ys], axis=-1)
        # in_ys:(5, 10, 6, 2, 257)
        in_ys = self.dense_fv(in_ys) # gw
        # in_ys:(5, 10, 6, 2, 256)
        
        #### Prediction Network ####
        # in_xs: (5, 120, 12, 256)
        p_xs = torch.tile(torch.unsqueeze(in_xs, axis=1), [1, N, 1, 1, 1])
        # p_xs: (5, 10, 120, 12, 256)
        que_x_1 = torch.unsqueeze(que_x, axis=-1)
        # que_x_1: (5, 10, 120, 12, 1)
        
        z = torch.concat([p_xs, que_x_1], axis=-1)
        # z: (5, 10, 120, 12, 257)
        ...
        # in_ys:(5, 10, 6, 2, 256)
        in_ys = torch.transpose(in_ys, 3, 2)
        # in_ys:(5, 10, 2, 6, 256)
        in_ys = torch.mean(in_ys, axis=-2)
        # in_ys:(5, 10, 2, 256)

        # out: (5, 10, 256)
        out = torch.unsqueeze(out, axis=2)
        # out: (5, 10, 1, 256)
        out = torch.tile(out, [1, 1, in_ys.shape[2], 1])
        # out: (5, 10, 2, 256)

        out = torch.concat([out, in_ys], -1)
        # out: (5, 10, 2, 512)
